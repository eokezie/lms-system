...
courseSchema.pre("validate", function (next) {
  ...
  // Ensure each embedded module has a moduleId, then enforce uniqueness **within this course only**.
  const modules = (this as any).courseModules as ICourseModule[] | undefined;
  if (Array.isArray(modules)) {
    const seen = new Set<string>();
    for (const mod of modules) {
      if (!mod) continue;
      if (mod.moduleId == null || mod.moduleId === "") {
        mod.moduleId = new mongoose.Types.ObjectId().toString();
      }
      const id = String(mod.moduleId).trim();
      if (seen.has(id)) {
        const err = new mongoose.Error.ValidationError(this as any);
        err.addError(
          "courseModules",
          new mongoose.Error.ValidatorError({
            message: `Duplicate moduleId "${id}" — each module in a course must have a unique moduleId`,
